...
public class TripPlannerDijkstra<NodeType, EdgeType extends Number>
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        ...
        if (vertices.get(end) == null || vertices.get(start) == null) {
            throw new NoSuchElementException();
        }

        PriorityQueue<Path> prioQueue = new PriorityQueue<Path>();

        Hashtable<Vertex, Boolean> booleanHash = new Hashtable();

        Hashtable<Vertex, Path> pathHash = new Hashtable();

        Vertex startVert = vertices.get(start);

        prioQueue.add(new Path(startVert));

        while (!prioQueue.isEmpty()) {
            Path nPath = prioQueue.poll();

            if (!pathHash.containsKey(nPath.end)) {
                booleanHash.put(nPath.end, true);
                pathHash.put(nPath.end, nPath);
                for (Edge e : nPath.end.edgesLeaving) {
                    prioQueue.add(new Path(nPath, e));
                }
            }
        }
        Vertex endVert = vertices.get(end);
        if (!pathHash.containsKey(endVert)) {
            throw new NoSuchElementException();
        }
        return pathHash.get(endVert); // TODO: Implement this method in Step 7.
    }
