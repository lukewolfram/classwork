...
void init_cache() {
    ...
    for (int setNum = 0; setNum < S; setNum++) {
        *(cache + setNum) = malloc(sizeof(cache_line_t) * E);
        if (*(cache + setNum) == NULL) {
            printf("Memory allocation failed\n");
            exit(1);
        }

        for (int lineNum; lineNum < E; lineNum++) {
            *(*(cache + setNum) + lineNum) = (cache_line_t) {'\0', 0, 0};
        }
    }
}
...
void access_data(mem_addr_t addr) {
    int setIndex; int tag; int setBits; int tagBits;
    int maxIndex = 0;
    int max = 0;

    setBits = ((int) pow(2, s) - 1) << b;
    tagBits = ((int) pow(2, 64 - (b + s)) - 1) << (b + s);

    setIndex = (addr & setBits) >> b;
    tag = (addr & tagBits) >> (b + s);

    cache_set_t currSet = *(cache + setIndex);
    cache_line_t currLine;

    for (int lineNum = 0; lineNum < E; lineNum++) {
        currLine = *(currSet + lineNum);

        if (currLine.valid == '\0') {
            (currSet + lineNum)->uses = 0;
            (currSet + lineNum)->tag = tag;
            (currSet + lineNum)->valid = '1';
        ...
        if (currLine.uses > max) {
            max = (*(cache + setIndex))->uses;
            maxIndex = lineNum;
        }
    }

    (currSet + maxIndex)->uses = 0;
    (currSet + maxIndex)->tag = tag;
